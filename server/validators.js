/**
 * Server-side Validators for Tipulon System
 */
...
export function validateBusNumber(busNumber) {
  ...
  // Cannot be all identical digits (e.g. 1111111, 2222222)
  if (new Set(clean).size === 1) {
    return 'מספר אוטובוס לא תקין: לא ניתן להזין רצף של ספרות זהות (כגון 2222222)';
  }

  // Cannot consist only of zeros and ones
  if (clean.split('').every(c => c === '0' || c === '1')) {
    return 'מספר אוטובוס לא תקין: לא ניתן להזין רק אפסים ואחדים';
  }

  // Cannot be dummy sequences
