...
export async function getLiveDepotsSnapshot() {
  ...
  const hubsResult = await Promise.all(HUBS.map(async (hub) => {
    ...
    return {
      ...
      lat: hub.lat,
      lon: hub.lon,
      isRestricted: Boolean(hub.isRestricted),
      type: hub.type || 'PARKING',
      warningText: hub.warningText || null,
      totalParkedCount: parkedBuses.length,
      availableForTreatmentCount: hub.isRestricted ? 0 : eligibleBusesForTreatment.length,
      busesForTreatment: hub.isRestricted ? [] : eligibleBusesForTreatment,
